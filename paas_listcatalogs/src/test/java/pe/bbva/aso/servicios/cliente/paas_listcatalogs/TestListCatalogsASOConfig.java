package pe.bbva.aso.servicios.cliente.paas_listcatalogs;
...
import pe.bbva.aso.servicios.listcatalogs.config.ListCatalogsASOConfig;
import pe.bbva.aso.servicios.listcatalogs.dto.CatalogsDTO;
...
public class TestListCatalogsASOConfig {
	...
	public void consultar() {
		...
		ListCatalogsASOConfig obj = new ListCatalogsASOConfig();
		RequestListCatalogs filtro = new RequestListCatalogs();
		
		ResponseListCatalogs response = null;
		List<CatalogsDTO> data = null;

		try {
			response = obj.listCatalogs(filtro, tsec);
			if(response!=null) {
				if(!response.hasError()) {
					data = response.getData();
					System.out.println("data "+ data.get(0).getValues().getId());
				}else {
