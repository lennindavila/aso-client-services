package pe.bbva.aso.servicios.cliente.paas_listaccounts;
...
import pe.bbva.aso.servicios.listaccounts.config.ListAccountsASOConfig;
import pe.bbva.aso.servicios.listaccounts.dto.AccountDTO;
...
public class TestListAccountsASOConfig {
	...
	public void consultar() {
		...
		ResponseListAccounts response = null;
		List<AccountDTO> data = null;

		try {
			response = obj.listAccounts(filtro, tsec);
			if(response!=null) {
				if(!response.hasError()) {
					data = response.getData();
					System.out.println("data "+ data.get(0).getAccountId());
				}else {
